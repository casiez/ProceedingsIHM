...
toc = open('archive-hal-annexe/toc.txt','w')

toc_out = open('archive-hal-annexe/toHal.xml','w')
...
for file in glob.glob(dir_file+"*.pdf"):
	...
	toc.write('Title: '+title+'\n')
	#print 'Authors: '+oneLineAuthor(authors_gname,authors_fname)
	toc.write('Authors: '+oneLineAuthor(authors_gname,authors_fname)+'\n')
	toc.write('Paper name: '+paper_basename+'.pdf\n')
	toc.write('Pages: '+str(curPage-nbPage)+'-'+str(curPage-1)+'\n\n')

	print 'ScriptsIHM14-Annexe/ScriptsIHM14-Annexe/PreparePDF -input '+file.replace(' ','\ ')+' -output archive-hal-annexe/'+paper_basename+'.pdf'+' -startpage '+str(curPage-nbPage)+' -title '+title[7:]+' -author '+oneLineAuthor(authors_gname,authors_fname)+" -conf IHM\\'14,\\ Villeneuve\\ d\\'Ascq,\\ France"+' -session '+session

	os.system('ScriptsIHM14-Annexe/ScriptsIHM14-Annexe/PreparePDF -input '
		+file.replace(' ','\ ')
		+' -output archive-hal-annexe/'+paper_basename+'.pdf'
		+' -startpage '+str(curPage-nbPage)+' -title '+title[7:]
		+' -author '+oneLineAuthor(authors_gname,authors_fname)
...
all_authors.sort()
for k in range(len(all_authors)):
	print all_authors[k] + ' ' + str(associate_pages[all_authors[k]])[1:-1]

os.system('cp Actes14/meta_data.txt archive-hal-annexe/meta_data.txt')
os.system('cp Actes14/frontmatter_annexe.pdf archive-hal-annexe/frontmatter.pdf')
os.system('cp Actes14/backmatter_annexe.pdf archive-hal-annexe/backmatter.pdf')
